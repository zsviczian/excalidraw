...
import { getFreeDrawSvgPath } from "../../element/src/freedrawPath";
...
const getThemeFilterValue = (theme: AppState["theme"]) =>
  theme === THEME.DARK ? DARK_THEME_FILTER : ""; // zsviczian
  ...
  switch (element.type) {
        ...
        if (typeof shape === "string") {
          ...
          const { pathD, fillColorThemed } = (() => {
            ...
            const fillColor = hasOutline
              ...
              : element.strokeColor;
            const fillColorThemed =
              renderConfig.theme === THEME.DARK
                ? applyDarkModeFilter(fillColor)
                : fillColor;

            if (hasOutline) {
              const outline = svgRoot.ownerDocument.createElementNS(
                SVG_NS,
                "path",
              );
              ...
              outline.setAttribute(
                "stroke-width",
                `${element.strokeWidth * outlineWidth}`,
              );
              outline.setAttribute("d", pathD);
              wrapper.appendChild(outline);
            }
            return { pathD, fillColorThemed };
          })();
    ...
    case "magicframe": {
      if (
        ...
        rect.setAttribute(
          "stroke-width",
          element.customData?.frameColor?.fill ?? //zsviczian
            renderConfig.frameColor?.fill ?? //zsviczian
            FRAME_STYLE.strokeWidth.toString(), //zsviczian
        );
