...
describe("Test <DropdownMenu/>", () => {
  ...
  it.each([
    ...
    async (_, triggerSelector) => {
      ...
      const portalRoot = container.ownerDocument.body; // zsviczian -- dropdown content is body-portaled in Obsidian
      ...
      for (const outsideTarget of [canvas, toolbar]) {
        for (const event of [fireEvent.pointerDown, fireEvent.touchStart]) {
          ...
          await waitFor(() => {
            expect(
              portalRoot.querySelector('[data-testid="dropdown-menu"]'),
            ).not.toBeNull();
          });
