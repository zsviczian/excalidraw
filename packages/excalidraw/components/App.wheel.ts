...
export class AppWheel {
  ...
  private isOverEditorSurface = (event: WheelEvent) => {
    ...
    const isOnExcalidrawCanvas =
      path?.some(
        (n) =>
          n instanceof ownerWindow.HTMLCanvasElement &&
  ...
  handle = withBatchedUpdates((event: WheelEvent) => {
    ...
    const { allowWheelZoom } = this.app.state;
    const hasZoomModifier = //zsviczian
      ((event.metaKey || event.ctrlKey) && !allowWheelZoom) ||
      (!(event.metaKey || event.ctrlKey) && allowWheelZoom)
    const shouldZoom =
