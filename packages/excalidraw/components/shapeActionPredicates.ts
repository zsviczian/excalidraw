...
export const getShapeActionPredicates = (
  ...
  //zsviczian
  const forFreedrawToolOrSelection = (
    predicate: (type: ElementOrToolType) => boolean,
  ) =>
    (predicate(activeToolType) && !appState.currentStrokeOptions) ||
    targetElements.some((element) => predicate(element.type) && !element.customData?.strokeOptions);
  ...
  return {
    ...
    frameRole: forToolOrSelection((type) => type === "frame"), //zsviczian
