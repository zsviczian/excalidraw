  ...
  it("uses host palettes for every target and keeps sticky top picks", () => {
    const customPalette: NonNullable<AppState["colorPalette"]> = {
      ...
      topPicks: {
        ...
        elementStroke: ["#333333", "#333333", "#333333", "#333333", "#333333"],
      },
    };
    ...
    const regular = resolveColorTarget(
      appState({
        ...
        selectedElementIds: { rectangle: true },
      }),
      elements,
      "strokeColor",
    );
    expect(regular.palette).toBe(customPalette.elementStroke);
    expect(regular.topPicks).toBe(customPalette.topPicks.elementStroke);

    const mixed = resolveColorTarget(
      appState({
        colorPalette: customPalette,
        selectedElementIds: { note: true, rectangle: true },
      }),
      elements,
      "backgroundColor",
    );
