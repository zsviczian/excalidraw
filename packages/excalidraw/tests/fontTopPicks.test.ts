...
describe("restore appState.fontTopPicks", () => {
  it("dedupes and drops unknown, internal and fallback font families", () => {
    const restored = restoreAppState(
      {
        fontTopPicks: [
          ...
          "7",
          FONT_FAMILY["Lilita One"],
          FONT_FAMILY["Local Font"], // zsviczian -- preserve the valid Obsidian-provided font family
    ...
    expect(restored.fontTopPicks).toEqual([
      FONT_FAMILY["Lilita One"],
      FONT_FAMILY["Local Font"], // zsviczian -- Local Font remains a valid customizable pick
      FONT_FAMILY.Virgil,
    ]);
  });
  ...
  it("caps at the strip slot count and nulls malformed input", () => {
    ...
    expect(
      restoreAppState({ fontTopPicks: [9999, "5"] } as any, null).fontTopPicks, // zsviczian -- 4 is the fork's valid Local Font id
    ).toBe(null);
  });
});
