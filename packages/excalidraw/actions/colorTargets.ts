...
export type ColorTargetAppState = Pick<
  ...
  | "editingTextElement"
  | "activeTool"
  | "colorPalette" // zsviczian -- Obsidian can replace the regular element palettes
  | ColorDefaultKey
>;
...
export const resolveColorTarget = (
  ...
  const regularPalette = isStroke
    ...
    : appState.colorPalette?.elementBackground ??
      DEFAULT_ELEMENT_BACKGROUND_COLOR_PALETTE;
  const regularTopPicks = isStroke
    ...
    topPicks:
      kind === "sticky"
        ? isStroke
          ? STICKY_NOTE_STROKE_PICKS
          : STICKY_NOTE_BACKGROUND_PICKS
        : regularTopPicks, // zsviczian -- mixed selections intentionally show the regular host picks
